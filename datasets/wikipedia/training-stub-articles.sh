xmlFile="enwiki-20180101-stub-articles.xml"
prefix="enwiki-articles"
...
#perl wikifil.pl $xmlFile >> "${prefix}-text.txt"
 ...
 #normalize_text < "${prefix}-text.txt"  | awk '{if (NF>1) print;}' >> "${prefix}-data.txt"

#../word2phrase -train "${prefix}-text.txt" -output "${prefix}-data-phrase.txt" -threshold 200 -debug 2
../word2phrase -train "${prefix}-data-phrase.txt" -output "${prefix}-data-phrase2.txt" -threshold 100 -debug 2
